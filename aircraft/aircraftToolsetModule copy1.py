...
class AircraftToolsetModule(AFXToolsetGui):
    ...
    def __init__(self):
        ...
        #初始化工具包
        FXMAPFUNC(self, SEL_COMMAND, self.ID_Ctest,   AircraftToolsetModule.onCmdCtest)  
        self.Ctest = CtestForm(self)

        #关联功能组件一
        FXMAPFUNC(self, SEL_COMMAND, self.ID_HONEYCOMB,  AircraftToolsetModule.onCmdHONEYCOMB)  
        self.HONEYCOMB = honeycombForm(self)
        #关联功能组件二
        FXMAPFUNC(self, SEL_COMMAND, self.ID_OVERSTRETHING,   AircraftToolsetModule.onCmdOVERSTRETHING)  
        self.OVERSTRETHING = overstrethingForm(self)
        #关联功能组件三
        FXMAPFUNC(self, SEL_COMMAND, self.ID_HELP,   AircraftToolsetModule.onCmdHelp)
        #关联功能组件四：帮助文档功能
        FXMAPFUNC(self, SEL_COMMAND, self.ID_FINDMATERIALS,  AircraftToolsetModule.onCmdFindMaterials)
        self.FINDMATERIALS=materialcheck_plugin(self)               
        #关联功能组件五：材料力学性能查询
        
        #
        #***************************************************************
        group = AFXToolbarGroup(self,'1结构设计分析工具1','1结构设计分析工具1')
        #创建工具条并命名为'结构设计分析工具'
        ...
        #增加功能组件三功能键
        AFXToolButton(p=group, label="\t材料库查询（上侧浮动）", 
        ...
        #创建菜单栏
        AFXMenuTitle(self, '自定义菜单下拉(test)', None, menu)
        #创建主菜单，'复合材料结构分析'
        ...
        #为飞出工具条创建工具箱
        VFrame_1 = FXVerticalFrame(p=group, opts=0, x=0, y=0,  w=0, h=0, pl=0, pr=0, pt=0, pb=0)
        #创建纵向排布框
